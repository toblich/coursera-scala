...
object LineOfSight {
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val (lt, rt) = parallel(upsweep(input, from, m, threshold), upsweep(input, m, end, threshold))
      Node(lt, rt)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int) {
    ...
    val firstAngle = if (from == 0) 0f else input(from) / from
    output(from) = max(startingAngle, firstAngle)
    var i = from + 1
    while (i < until) {
