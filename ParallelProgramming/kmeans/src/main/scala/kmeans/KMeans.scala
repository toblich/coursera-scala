...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    val unassigned = for (mean <- means filterNot classified.contains) yield (mean, Nil)
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    if (hasConverged) newMeans else kMeans(points, newMeans, eta) // your implementation need to be tail recursive
  }
}
