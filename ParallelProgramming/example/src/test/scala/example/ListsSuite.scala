 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of single-element list is element") {
    assert(sum(List(5)) === 5)
  }

  test("sum of long list") {
    assert(sum(List(1,2,4,-1,7,-5)) === 1+2+4-1+7-5)
  }

  test("max of empty list throws NoSuchElement exception") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("max of single-element list is element") {
    assert{ max(List(2)) === 2 }
  }

  test("max of two elements (first)") {
    assert { max(List(1,-1)) === 1 }
  }

  test("max of two elements (second)") {
    assert { max(List(1,2)) === 2 }
  }

  test("max of long list") {
    assert { max(List(1,2,4,-4,1,-2,3,0,-7)) === 4 }
  }
